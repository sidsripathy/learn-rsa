...
export default function InstitutePage({ params }: { params: { slug: string } }) {
  ...
  return (
    <section className="mx-auto w-full max-w-6xl px-6 py-16">
      <div className="mb-12">
        <Link href="/all-iiits" className="text-sm text-slate-300 transition hover:text-white">
          ← Back to directory
        </Link>
        <h1 className="mt-4 text-4xl font-semibold text-white">{institute.shortName}</h1>
        <p className="mt-3 text-base text-slate-300">{institute.summary}</p>
        <p className="mt-3 text-xs font-semibold uppercase tracking-[0.25em] text-slate-400">{institute.region}</p>
      </div>
            ...
            <NeonCard key={person.email}>
              <div className="flex flex-col items-center gap-4 text-center md:flex-row md:text-left">
                ...
                <div>
                  ...
                  {person.tagline && (
                    <p className="mt-3 text-xs uppercase tracking-[0.25em] text-emerald-300/80">{person.tagline}</p>
                  )}
              ...
              <div className="space-y-6">
                <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                    ...
                    {team.awards && (
                      <div className="mt-3 flex flex-wrap gap-2">
                        {team.awards.map((award) => (
                          <span
                            key={award}
                            className="rounded-full border border-white/20 bg-white/10 px-3 py-1 text-xs uppercase tracking-[0.25em] text-slate-200"
                          >
                            {award}
                          </span>
                        ))}
                  ...
                  <div className="text-right text-sm text-slate-300">
                    <span className="text-xs uppercase tracking-[0.25em] text-slate-400">Project focus</span>
                    <p className="mt-1 font-medium text-white">{team.project.title}</p>
                  </div>
                </div>
                ...
                <div className="grid gap-6 md:grid-cols-2">
                  <div className="space-y-3 text-sm text-slate-300">
                    ...
                    <h4 className="text-base font-semibold text-white">Project narrative</h4>
                    <p>{team.project.description}</p>
                    <div>
                      <h4 className="text-base font-semibold text-white">Tech stack</h4>
                      <div className="mt-2 flex flex-wrap gap-2 text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">
                        {team.project.techStack.map((tech) => (
                          <span key={tech} className="rounded-full border border-white/20 px-3 py-1 text-slate-200">
                            {tech}
                          </span>
                        ))}
                      </div>
                    ...
                    <div>
                      <h4 className="text-base font-semibold text-white">Highlights</h4>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid gap-3">
                      {team.members.map((member) => (
                        <div key={member.name} className="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/5 p-3">
                          ...
                          <div>
                            ...
                            {member.tagline && <p className="text-xs text-slate-300">{member.tagline}</p>}
                          </div>
                        </div>
                      ))}
                    </div>
