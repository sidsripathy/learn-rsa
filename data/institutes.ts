...
export const institutes: Institute[] = baseInstitutes.map((inst, index) => {
  ...
  const teamTemplate = teamTemplates[index % teamTemplates.length];
  ...
  const team: Team = {
    name: `${teamTemplate.teamBase} ${inst.shortName.split(' ').pop()}`,
    track: teamTemplate.track,
    round: ((index % 3) + 1) as 1 | 2 | 3,
    project: teamTemplate.project,
    members: teamTemplate.members.map((member, memberIndex) => ({
      ...member,
      photo: `https://i.pravatar.cc/160?img=${((index * 5 + memberIndex) % 70) + 1}`
    })),
    awards: index % 5 === 0 ? ['Jury Spotlight'] : undefined
  };
