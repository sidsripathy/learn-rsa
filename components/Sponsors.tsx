      ...
      {sponsorGroups.map((group) => (
        <NeonCard key={group.tier}>
          <div className="space-y-5">
            <p className="text-sm font-medium uppercase tracking-[0.2em] text-slate-400">{group.tier}</p>
            <div className="space-y-4">
              {group.sponsors.map((sponsor) => (
                <div key={sponsor.name} className="flex items-center gap-4 rounded-2xl border border-white/10 bg-white/5 p-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-slate-300">{sponsor.tagline}</p>
                  </div>
                </div>
              ))}
            </div>
